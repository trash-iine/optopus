...
/// score per option, and on every `segment_len`-th record it folds each
/// option's *average* score for the segment into its weight as
/// `w ← (1 − reaction) · w + reaction · avg`, then clears the segment.
///
/// The blend is part of recording rather than a second call the caller has to
/// remember. **A caller that scored without ever blending would get a wheel
/// that stayed uniform for the whole run**, which is a working search that has
/// silently stopped being adaptive.
///
...
pub struct AdaptiveWeights {
    weights: Vec<f64>,
    scores: Vec<f64>,
    /// Uses of each option this segment. Their sum is how far into the
    /// segment the wheel is.
    counts: Vec<u64>,
    segment_len: u64,
    reaction: f64,
}
...
impl AdaptiveWeights {
    ...
    pub fn select(&self, rng: &mut SmallRng) -> usize {
        ...
        // subtraction, where the last option is the one the wheel had landed
        // on.
        self.weights.len() - 1
    }

    /// Credits `option` with `score`, and at a segment boundary blends the
    /// segment's average scores into the weights.
    pub fn record(&mut self, option: usize, score: f64) {
        self.scores[option] += score;
        self.counts[option] += 1;
        if self.counts.iter().sum::<u64>() < self.segment_len {
            return;
        }
...
mod tests {
    ...
    fn a_segment_boundary_blends_the_average_in() {
        ...
        // The segment restarted, so the next four uses are blended on their
        // own average rather than on top of the one just folded in. A segment
        // that scores badly has to pull the weight back down, or the wheel
        // could only ever climb.
        let climbed = w.weights()[0];
        for _ in 0..4 {
            w.record(0, 0.5);
        }
        assert_eq!(w.weights()[0], 0.5 * climbed + 0.5 * 0.5);
        assert!(
            w.weights()[0] < climbed,
    ...
    /// With every weight equal the draw is uniform, which is the other half of
    /// what "proportional" means and the half a wheel that ignored its weights
    /// entirely would still pass. Three thousand draws over three options land
    /// within a few percent of a thousand each.
    #[test]
    fn a_uniform_wheel_draws_uniformly() {
        let w = AdaptiveWeights::new(3, 100, 0.1);
        let mut rng = rng();
        let mut counts = [0usize; 3];
        for _ in 0..3_000 {
            counts[w.select(&mut rng)] += 1;
        }
        for (i, &c) in counts.iter().enumerate() {
            assert!(
                (800..1_200).contains(&c),
                "option {i} drawn {c} times of 3000"
            );
        }
    }
    ...
    /// and the wheel must still return a valid option rather than divide by
    /// zero.
    #[test]
