...
//! places), and routing `insertion_cost` through a vtable there would be paid
...
/// textbook operator, and also `k` times the work, while two adjacent elements
...
pub fn worst_removal<P: Ruinable>(
    ...
    prob.elements(partial, scratch);
    remove_top_k(prob, partial, k, scratch, |prob, partial, e| {
        // Negated so the shared routine's ascending sort puts the largest gain
        // first. Negation reverses `total_cmp`'s order exactly, signed zeros
        // included, so this selects the same elements a descending comparator
        // would, and one sort direction serves both operators.
        -prob.removal_gain(partial, e)
    })
}

/// Removes the `k` elements most related to one random seed element (Shaw).
///
/// Taking a *cluster* is the point, because a scattered removal can usually be
/// undone by putting everything back where it was, while a cluster leaves room
/// for a genuinely different arrangement.
pub fn shaw_removal<P: Ruinable>(
    ...
    remove_top_k(prob, partial, k, scratch, move |prob, _, e| {
        prob.relatedness(seed, e)
    })
}

/// Scores every element of `placed`, removes the `k` cheapest by that score,
/// and returns them in the order they were scored.
///
/// The caller supplies the listing rather than this asking for one, because
/// Shaw removal has to draw its seed from it first and would otherwise pay for
/// the same [`Ruinable::elements`] pass twice.
///
/// The sort is stable and always ascending, so two elements that score the
/// same are removed in the order [`Ruinable::elements`] listed them. Both
/// destroy operators that rank go through here, which is what keeps that
/// tie-breaking one decision rather than two.
fn remove_top_k<P: Ruinable>(
    prob: &P,
    partial: &mut P::Partial,
    k: usize,
    placed: &[P::Element],
    score: impl Fn(&P, &P::Partial, P::Element) -> f64,
) -> Vec<P::Element> {
    let mut scored: Vec<(f64, P::Element)> = placed
        .iter()
        .map(|&e| (score(prob, partial, e), e))
        .collect();
...
///
/// With a single container the second entry stays infinite, which the caller
/// reads as unbounded regret.
...
/// Re-inserts elements largest-regret-first, so the element that stands to lose
