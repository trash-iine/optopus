...
- **Run configs**: TOMLs under `data/benchmarks/<problem>/{general,lkh,hgs}_{small,medium,large}.toml`
  (`lkh_*` is the TSP-specific track, `hgs_*` pits the two bespoke CVRP
  heuristics against each other on equal budget)
