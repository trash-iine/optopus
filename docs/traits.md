...
| `BinaryProblem` | the shared binary machinery in `common::binary`, `SearchState::close_reduction` | `type Flip;`<br>`fn variable_indices(&self) -> Range<usize>;`<br>`fn variable(sol, i) -> bool;`<br>`fn assignment(sol) -> &[bool];`<br>`fn flip_move(sol, i) -> Self::Flip` |
