...
| `Evaluate<T>` (on the move) | `SimulatedAnnealing`, `BangBangSimulatedAnnealing`, `LateAcceptanceHillClimbing`, `RlSearch` (the last also needs `Clone` on the move) | `fn evaluate(&self) -> Evaluable<T>` (default `T = f64`), the change applying the move would make. `Evaluable::Maximize(T)` / `Minimize(T)` carries the optimization direction, and `Evaluable<f64>::minimized()` applies it so that lower is better (`worsening_amount()` is the same number, named for a delta). |
| `Evaluate<f64>` (on `Solution`) | `PopulationAnnealing`, and registering the problem with the benchmark | same signature, reporting the solution's own objective rather than a change. Needed by a search that computes with the objective rather than only comparing them. |
