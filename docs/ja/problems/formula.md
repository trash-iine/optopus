...
この表は、各変数が変わりうる値ごとに 1 つの数を持つので、大きさは変数ごとの `upper - lower` の合計です。
数百万通りの値をとる変数があると、どの解もその大きさになります。そういう変数には、差分を書いた [`IntegerProblem`](integer.md) のほうが向いています。
順列には一つの変数だけを変える近傍がないので、表は持たず、Swap と Reverse は単項式から直接求めます。
