...
all three moved nothing, so the partial is built once per iteration and the
trait has no refill method. The count and the index stayed because their
overrides are one line each.
