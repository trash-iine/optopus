...
### Problem-specific
MaxCut has its own directory (`specific/max_cut/`) for `BreakoutLocalSearchForMaxCut` and `PopulationAnnealingForMaxCut`. Almost nothing there is a bespoke operator any more: three quarters of a BLS round are the library's generic heuristics — the descent is a `LocalSearch`, the strong perturbation a `RandomWalk`, the weak flip a `TabuSearch`, all driven from `bls.rs` — which works because recording is a mode on the state (`start_record_tabu`), so their `apply` writes the same prohibitions a hand-written operator would. Only `best_swap.rs` is left, because `M2` moves one vertex per partition side in a *single* move, which two independent one-step searches cannot express. The substitutions were measured, not assumed: the descent cost **1.15x at equal iterations** (−41.8 cut points at 30s over 10 G-set instances) and was taken deliberately; the weak flip **gained +318.2 (7-0) and 1.8x speed**, by a mechanism that is recorded as unexplained in `docs/heuristics/breakout_local_search.md` rather than guessed at. `bls.rs` holds the selection vocabulary (`PerturbationType`, re-exported as `MaxCutPerturbation`) and the one match that maps it to an operator, `BreakoutLocalSearch::kick`.
...
1. **Gain-based incremental updates** — binary/formula solutions cache per-variable `gain`; applying a move only refreshes the affected neighbors in O(degree). There are no auxiliary gain indexes any more: MaxCut and QUBO each carried optional incrementally maintained sets of the improving or plateau variables, and all three were deleted once nothing read them — the last, MaxCut's `zero_gain`, when `PopulationAnnealingForMaxCut`'s cluster move switched to scanning `gain` and got 0.92-0.94x faster for it. TSP instead computes move gains on the fly from the lazily built distance matrix; JobShop re-decodes per candidate (and evaluates candidates with rayon on large instances, order-preserving so results are thread-count independent).
