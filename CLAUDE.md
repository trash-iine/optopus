...
Measured decisions live in `decisions/`, one file per decision with an index in
`decisions/README.md`. Read that index before re-proposing anything that looks
like an obvious improvement; several of them were tried and priced already.
`docs/` and the rustdoc comments are for users of the library and carry none of
that material.
...
│       ├── max_cut/
│       │   ├── best_swap.rs    the one operator with no generic equivalent:
│       │   │                    Benlic & Hao's M2, a free fn over the
│       │   │                    SearchState's tabu memory (+ the keep_best
│       │   │                    tie rule it selects with)
...
**Tabu memory** — `apply` / `apply_move_only` record the applied move in the state's tabu memory, at the iteration it was made on, before the counter advances, **but only while the state is recording**. Recording is off by default and off in every sub-run, because most searches never read that memory and writing it costs an RNG draw and a store per move. A search whose method *is* the tabu list turns it on with `start_record_tabu((min,max))`, which sets the tenure and the mode in one call because they were always one decision — `TabuSearch::run_once` and `BreakoutLocalSearch::prepare` both do it, per iteration and per half-round, so the mode is never set far from the loop that depends on it. That is why no heuristic pairs `record` with `apply` by hand, and why operators that must respect each other's prohibitions get that from sharing a state rather than from being handed the same object. `start_record_tabu((min,max))` (until armed the tenure is `(0,0)` = record but forbid nothing) / `tabu_tenure()` / `reset_tabu()` / `tabu_allows(&mv) -> bool` (typed on the move, so `is_move_enabled` inlines into a neighborhood scan and a move without tabu cannot compile a call) / `stop_record_tabu()` (keeps the tenure) / `is_recording_tabu()` / `record_tabu(&mv)` (typed too; for forbidding a move that is *not* being applied) / `reserve_tabu_vars(n)` (pre-allocation only). `TabuMemory` keys by `TabuKey` shape, so MaxCut's flip and swap (both `Var`) share prohibitions — the property BLS is built on — while JobShop's swap (`Var`) and relocate (`Pair`) do not. `clone_for_new_run` gives the child an **empty** memory under every clone type (a phase is its own tabu list), and `update_state` carries the sub-run's prohibitions *back* into the parent along with its solution — a replacement, not a merge, and sound only because parent and child share one iteration frame. `sub.inherit_tabu_from(&parent)` is the opposite direction, for a phase that should start from what the parent knows.
...
| `TabuSearch<N>` | Best non-tabu neighbor; aspiration overrides tabu when global best is improved; tenure ∈ `(min, max)` sampled from `state.rng`. Holds the tenure only — the map is on the state, and `run_once` arms `start_record_tabu` beside the tenure so `apply` records |
...
### Problem-specific
MaxCut has its own directory (`specific/max_cut/`) for `BreakoutLocalSearchForMaxCut` and `PopulationAnnealingForMaxCut`. Three quarters of a BLS round are the library's generic heuristics, a `LocalSearch` descent, a `RandomWalk` strong perturbation and a `TabuSearch` weak flip, all driven from `bls.rs`; that works because recording is a mode on the state (`start_record_tabu`), so their `apply` writes the same prohibitions a hand-written operator would. Only `best_swap.rs` is a bespoke operator, because `M2` moves one vertex per partition side in a single move. `bls.rs` holds the selection vocabulary (`PerturbationType`, re-exported as `MaxCutPerturbation`) and the one match that maps it to an operator, `BreakoutLocalSearch::kick`. See `decisions/0001`.
...
- The learned perturbation controller is an example, not a heuristic. `examples/rl_bls.rs` drives `BreakoutLocalSearchForMaxCut` through `descend` / `kick` (constructed with `externally_driven`, which takes the tenure literally instead of as the paper's γ) and picks perturbation type times strength with a contextual softmax bandit (`reinforcement_learning::bandit::SoftmaxBandit`); weights persist across `Restart` / `Iterated` episodes. It was a heuristic with a benchmark `kind` until 2026-08-31, and moved out whole because everything it added to BLS was policy. Two plateau operators were dropped from its action space; see `decisions/0007`. See `docs/guide/learned_perturbation.md`.
- Kernelization is not a heuristic and has no wrapper type. `MaxCutKernel::new` (`problem/max_cut/kernel.rs`) produces an exactly reduced instance using the isolated, pendant, degree-2-path and weight-domination rules (arXiv:1905.10902) to a fixpoint, with a trace that lifts a kernel solution back, and any `Heuristic<MaxCut>` searches it through `open_reduction` / `close_reduction`. Reduction is a property of the instance, and `is_trivial()` says in one comparison whether the crossing is worth making; see `decisions/0009`. Details in `docs/problems/max_cut_kernel.md`.
...
VRP has its own directory (`specific/vrp/`) because what ALNS and HGS share is the *route machinery*, not merely the problem type — the reason MaxCut's `ops/` no longer exists, and this one still does. It lives in `ops/` — the pricing free functions (a route edit's distance and overload deltas), `RouteState`, the granular candidate lists, and `Descent`. `Descent` is the one thing there with a receiver, because it owns the caches (candidate lists + sweep buffers) both heuristics were otherwise keeping a private copy of; the pricing stays free functions because the two callers hold their routes in different containers. What no `ops` item decides is *policy*: when to descend, under which penalty, and over which customers is the caller's, which is exactly why one descent serves a fixed penalty and an adapted one.

- `AdaptiveLargeNeighborhoodSearchForVrp` (`specific/vrp/alns.rs`): ALNS ruin-and-recreate for CVRP. An `AlnsOps` bank (destroy: random / worst / Shaw removal; repair: greedy / regret-2 insertion) with adaptive roulette-wheel operator weights (segment-updated) and SA acceptance; operates directly on `state.solution` like LKH. Each recreated solution then runs through `ops::Descent::run_around`, anchored at the re-inserted customers under `Vrp::penalty_weight()`; the anchoring is what makes it pay, see `decisions/0012`. VRP-only, via `build_special_heuristic`.
- `HybridGeneticSearchForVrp` (`specific/vrp/hgs/`): giant-tour GA. OX crossover, then `split_giant_tour` for an optimal decode, then the shared granular descent (relocate / swap / 2-opt / 2-opt\*, restricted to the Γ nearest partners) over every customer of the offspring. Keeps feasible and infeasible sub-populations ranked by biased fitness (cost rank plus broken-pairs diversity rank), with a capacity penalty retuned to hold the feasible share near `target_feasible`. It deliberately does not use the `Vrp*Neighbor` types or `Vrp::penalty_weight()`, which bake in a fixed enormous penalty that is incompatible with a penalty the search adapts; `Vrp::solution_from_routes` converts back only when writing to the state. It no longer beats ALNS at equal budget, see `decisions/0008`.
...
|---|---|---|---|---|---|
| **MaxCut** | Max | `x`, `gain: Vec<f32>`, `objective: f32` | Flip / Swap | Uniform | format: `N M / i j w` (1-indexed) |
...
MaxCut instance suites. Besides the G-set, three suites are generated locally from fixed seeds and never committed, since regenerating reproduces them byte for byte. `examples/generate_dense_maxcut.rs` covers the 10 to 30 percent density band the G-set does not reach, `generate_sparse_maxcut.rs` covers average degree 1 to 5 where `MaxCutKernel` fires, and `generate_hard_maxcut.rs` writes `data/instances/max_cut/hard/`, whose optimum is exact by construction (`PlantedMaxCut`, following `chook`, arXiv:2005.14344). The planted suite is the only one where a gap is measured against the answer rather than against another heuristic's best result. Its optima live in `hard/manifest.toml`, which nothing in the library reads, the same rule as best-known values. Hardness parameters and why published values do not transfer are in `decisions/0010`.
...
1. Gain-based incremental updates. Binary and formula solutions cache a per-variable `gain`, so applying a move only refreshes the affected neighbors in O(degree). There are no auxiliary gain indexes; see `decisions/0002`. TSP instead computes move gains on the fly from the lazily built distance matrix, and JobShop re-decodes per candidate, evaluating candidates with rayon on large instances in an order-preserving way so results are thread-count independent.
